...
def is_target_destination(url):
    ...
    try:
        ...
        # Exclude known intermediate safelink slugs or non-episode system pages
        if any(bad in path_lower for bad in [
            "choose-best", "web-hosting", "seo-tips", "sitemap",
            "privacy", "cookie", "about", "terms", "contact"
        ]):
            return False
...
def extract_safelink_bypass(html, current_url):
    ...
    if target_match:
        ...
        if parsed_cand.hostname and not is_blocked_host(parsed_cand.hostname) and is_target_destination(target_cand) and target_cand != current_url.split("?")[0]:
            return target_cand
    ...
    # If raw_url_param decodes directly to a valid destination URL or shortcode
    if raw_url_param:
        try:
            ...
            if decoded_try.startswith(("http://", "https://")):
                ...
                if p.hostname and not is_blocked_host(p.hostname) and p.hostname != parsed_cur.hostname:
                    return decoded_try
            # If decoded_try is an alphanumeric code (e.g. "Ij03ndJ", "pu5w"), route to gateway
            elif re.match(r'^[a-zA-Z0-9_-]{3,25}$', decoded_try):
                return f"https://go.sohojgyan.com/{decoded_try}"
        except Exception:
            pass
...
class FormInputParser(HTMLParser):
    ...
    def handle_starttag(self, tag, attrs):
        if tag.lower() == "input":
            ...
            name = d.get("name")
            if name:
                # Do NOT unquote values here, CakePHP token signatures rely on the exact literal string
                self.inputs.append((name, d.get("value", "")))
...
def extract_adlinkfly_bypass(html, current_url, cookie_jar, opener):
    ...
    try:
        ...
        # Fallback regex
        form_data = re.findall(r'<input[^>]+name=["\']([^"\']+)["\'][^>]*value=["\']([^"\']*)["\']', form_html)
...
def resolve_url(start_url, max_redirects=MAX_REDIRECTS, return_html=False):
    ...
    for number in range(1, MAX_REDIRECTS + 1):
        ...
        # -------------------------------------------------
        # 0. Check if already at target destination
        # -------------------------------------------------
        if is_target_destination(current_url):
            chain.append({
                "step": number,
            ...
            if return_html:
                try:
                    req_dest = Request(current_url, headers={
                        "User-Agent": USER_AGENT,
                        "Accept": "text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8",
                    })
                    resp_dest = opener.open(req_dest, timeout=TIMEOUT)
                    raw_dest = resp_dest.read(MAX_HTML_BYTES)
                    raw_dest = decompress_body(raw_dest, resp_dest.headers.get("Content-Encoding", ""))
                    html = raw_dest.decode("utf-8", errors="replace")
                    resp_dest.close()
                except Exception:
                    pass
            break
        ...
        if "shrt.sohojgyan" in cur_host:
            ...
            if code and len(code) >= 3 and not is_target_destination(current_url):
                go_url = f"https://go.sohojgyan.com/{code}"
                chain.append({
                    "step": number,
                    "url": current_url,
                    "status": 302,
...
def resolve_shortlink_until_target(start_url, max_retries=4, return_html=False):
    """
    Resolves the shortened URL with retry logic.
    The resolved/final destination of shorten url should be this structure/format:
    (https://mydverse02.blogspot.com/p/flp-120926.html, https://mydverse02.blogspot.com/p/mbmb-030826.html).
    Retries until the resolved/final destination matches this structure/format.
    """
    start_url = validate_url(start_url)
    last_res = None
    ...
    for attempt in range(1, max_retries + 1):
        try:
            # On attempt > 1, try direct gateway variant if shrt.sohojgyan
            current_target = start_url
            if attempt == 2 and "shrt.sohojgyan" in start_url.lower():
                code = urlparse(start_url).path.strip("/").split("/")[-1]
                if code and len(code) >= 3:
                    current_target = f"https://go.sohojgyan.com/{code}"

            res = resolve_url(current_target, return_html=return_html)
            last_res = res
            final_url = res.get("final", "")

            # Verify if destination matches target Blogspot structure/format
            if is_target_destination(final_url):
                res["attempts"] = attempt
                res["target_destination_verified"] = True
                return res
    ...
    if last_res is not None:
        ...
        last_res["target_destination_verified"] = is_target_destination(last_res.get("final", ""))
        return last_res

    raise RuntimeError(f"Failed to resolve shortened URL after {max_retries} attempts.")
...
def run_resolver_api(url):
    ...
    try:
        # Resolve shortlink with retry until the resolved destination matches Blogspot structure
        result = resolve_shortlink_until_target(url, max_retries=4, return_html=False)
