...
export const EpisodeButtonsGenerator: React.FC<EpisodeButtonsGeneratorProps> = ({
  ...
  const generatedHtml = useMemo(() => {
    ...
    const buttonsHtml = buttonsData
      .map((btn) => {
        if (btn.isFilled) {
          // First button: Filled Blue (350px width, 1 per row, no icon)
          return `  <a href="${btn.url}"${targetAttr} class="ep-btn ep-btn-filled" style="width: 350px; max-width: 100%; box-sizing: border-box; display: inline-flex; align-items: center; justify-content: center; padding: 12px 20px; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; font-size: 15px; font-weight: 600; text-align: center; text-decoration: none; border-radius: 8px; transition: all 0.2s ease; background-color: #2563eb; color: #ffffff; border: 1.5px solid #2563eb; box-shadow: 0 2px 5px rgba(37, 99, 235, 0.25); cursor: pointer;">${btn.label}</a>`;
        } else {
          // Second button: Outlined Blue with low filled color opacity (350px width, 1 per row, no icon)
          return `  <a href="${btn.url}"${targetAttr} class="ep-btn ep-btn-outlined" style="width: 350px; max-width: 100%; box-sizing: border-box; display: inline-flex; align-items: center; justify-content: center; padding: 12px 20px; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; font-size: 15px; font-weight: 600; text-align: center; text-decoration: none; border-radius: 8px; transition: all 0.2s ease; background-color: rgba(37, 99, 235, 0.08); color: #2563eb; border: 1.5px solid #2563eb; cursor: pointer;">${btn.label}</a>`;
        }
      })
      .join("\n");
    ...
    if (includeHoverStyle) {
      return `<!-- Episode Buttons (350px Width, One Per Row, Blue Combo: Alternating Filled & Outlined) -->
<style>
  .episodes-container {
    display: flex;
...
${buttonsHtml}${sessionEndHtml}
    ...
    return `<!-- Episode Buttons (350px Width, One Per Row, Pure Inline HTML) -->
<div style="display: flex; flex-direction: column; align-items: center; gap: 12px; width: 100%; margin: 16px 0;">
${buttonsHtml}${sessionEndHtml}
</div>`;
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <p className="text-xs text-slate-600">
              350px width • 1 button per row • Alternating filled &amp; outlined • No icons • Red &ldquo;- Session End -&rdquo; footer
            </p>
          </div>
        </div>
          ...
          {activeTab === "preview" && (
            <div className="bg-white rounded-xl p-4 sm:p-5 border border-slate-200/80 shadow-inner">
              <div className="text-[11px] font-semibold text-slate-400 uppercase tracking-wider mb-3 flex items-center justify-between">
                ...
                <span className="text-blue-600 normal-case font-normal text-xs">
                  Click any button to test link in new tab
                </span>
              </div>

              {/* Render the buttons according to specification: 350px width, 1 per row, no icons */}
                  ...
                  if (btn.isFilled) {
                    return (
                      <a
                        ...
                        className="w-[350px] max-w-full inline-flex items-center justify-center px-5 py-2.5 rounded-lg text-sm font-semibold transition-all duration-200 cursor-pointer shadow-sm hover:shadow-md hover:-translate-y-0.5 active:scale-95 select-none text-center"
                        style={{
                    ...
                    return (
                      <a
                        ...
                        target={openInNewTab ? "_blank" : undefined}
                        rel={openInNewTab ? "noopener noreferrer" : undefined}
                        className="w-[350px] max-w-full inline-flex items-center justify-center px-5 py-2.5 rounded-lg text-sm font-semibold transition-all duration-200 cursor-pointer hover:shadow-xs hover:-translate-y-0.5 active:scale-95 select-none text-center"
                        style={{
